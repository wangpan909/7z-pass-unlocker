...
资源圈常见的"双形态文件"：文件本身能当视频播放（MP4 伪装层），
但末尾真正藏着一个加密 zip（如内嵌 `xxx.jpg` 密码图）。这类文件
7-Zip 直接打开会报"不是压缩包"（本地文件头偏移对不上），但
Bandizip 能解。本工具已内置**伪装识别**：7z 报"打不开/不是压缩包"
时，自动扫描文件尾部 EOCD 定位内嵌 zip 起点，剥离前置数据到临时
文件后重试解压，成功后自动清理临时文件——用户无感知。

> 例如 `教程 (11)_1.zip` 实为 MP4 视频 + 内嵌加密 zip，密码
> `xxld`，工具现已可直接解出内嵌的 `乐园日常.jpg`。

## 中文/日文密码双形态尝试

7z.exe 命令行 `-p` 传非 ASCII 密码**不稳定**：同一字符串有时按
标准 Unicode 存储、有时按系统代码页（GBK）转换后存储，两种形态
互相对不上，导致"同样密码时而成功时而报错"。本工具对含非 ASCII
字符的密码**依次尝试两种形态**：

1. `-P` 密码文件（7-Zip 24.00+，按 UTF-8 读取）——与 7-Zip GUI /
   Bandizip / WinRAR 等常规工具创建的包一致；
2. `-p` 命令行形态——兼容经代码页转换创建的老式 CLI 包。

纯 ASCII 密码不受影响，仍只尝试一次。密码文本文件读取同时支持
UTF-8 / GBK / UTF-16（BOM）。
